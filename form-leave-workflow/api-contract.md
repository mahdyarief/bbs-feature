...
> **Endpoint baru:** `PATCH /v1/teacher-leaves/:id/status` — ditambahkan ke controller `teacher-leave` yang sudah ada dari fase 1.
> **Response extend:** `GET /v1/teacher-leaves` dan `GET /v1/teacher-leaves/:id` dari fase 1 sekarang menyertakan field status approval.
> Semua endpoint memerlukan JWT Bearer token (global `JwtAuthGuard`). Permission di-enforce via `@CheckPermissions` dengan subject `ModulesTypeEnum.TEACHER_LEAVE`.
...
### 1. PATCH /v1/teacher-leaves/:id/status — Ubah status + komentar
...
### 2. GET /v1/teacher-leaves — List (extend fase 1)
...
### 3. GET /v1/teacher-leaves/:id — Detail (extend fase 1)
...
- State machine pattern: validasi transisi via map object `{ [from: string]: string[] }` — contoh:
  ```ts
  const VALID_TRANSITIONS: Record<TeacherLeaveStatusEnum, TeacherLeaveStatusEnum[]> = {
    [TeacherLeaveStatusEnum.PENDING]: [TeacherLeaveStatusEnum.APPROVED_BY_ADMIN, TeacherLeaveStatusEnum.REJECTED],
    [TeacherLeaveStatusEnum.APPROVED_BY_ADMIN]: [TeacherLeaveStatusEnum.APPROVED_BY_PRINCIPAL, TeacherLeaveStatusEnum.REJECTED],
    [TeacherLeaveStatusEnum.APPROVED_BY_PRINCIPAL]: [],
    [TeacherLeaveStatusEnum.REJECTED]: [],
  };
  ```
- Controller pattern: `src/modules/lesson/lesson.controller.ts` — `@Patch(':id')`, `@CheckPermissions`, `@Req() req: Request`, `ParseIntPipe`.
